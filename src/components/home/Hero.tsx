 ...
 * background and lash curves sit behind. On mobile the composition reorders
 * (headline → image → CTA) so type dominates without tiny side text (§40).
 * Hero image is priority/LCP (§29); fades in once loaded (§49).
 */
export default function Hero({ media }: { media: HeroMedia }) {
  ...
  return (
    <section className="relative flex min-h-[90svh] flex-col overflow-hidden bg-ink-black text-warm-white">
      ...
      <div className="relative z-10 mx-auto grid w-full max-w-7xl flex-1 grid-cols-1 items-center gap-10 px-4 pb-16 pt-28 sm:px-6 md:grid-cols-[1.15fr_0.85fr] md:gap-6 md:pt-24 lg:px-8">
        {/* Copy column */}
        <div className="order-2 md:order-1">
          <motion.p
            ...
            className="font-editorial mb-6 text-rose-muted md:mb-8"
            style={{
              fontSize: 'clamp(1.375rem, 2.6vw, 2.5rem)',
          ...
          <motion.p
            ...
            className="font-sans-ui mt-6 max-w-md text-sm md:text-base text-warm-white/70"
          >
            {BRAND.tagline}
          </motion.p>
        ...
        <motion.div
          ...
          className="relative order-1 md:order-2 md:justify-self-end"
        >
          <div className="relative -rotate-1 md:w-[26rem] lg:w-[30rem]">
            <SmartImage
              src={media.src}
              alt={media.alt}
              aspectRatio="4 / 5"
